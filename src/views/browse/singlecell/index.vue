...
const id1 = ref('')
onMounted(() => {
  const id = route.query?.id; // 获取参数 id
  // console.log(id);
  if (id == "1") {
    id1.value = id
    // loading.value = true;
    // dataset.value = "GSE171668_Airway";
    // browsefiesttable.value = false;
    // nextTick(() => {
    //   el.value =
    //     document.querySelector<HTMLElement>(".app-main .el-scrollbar__wrap") ??
...
<template>
  <div class="contentheight">
    <el-row :gutter="24" class="mb-[18px] contentstart" v-loading="loading" element-loading-text="Loading...">
  ...
  transition: {
    ...
    delay: 200
        ...
        <el-card shadow="never" class="min-h-[780px]">
          ...
          <template #default>
            <tree ref="treeref" :Data="id1" class="w-[100%] float-left" />
          </template>
          <!-- </el-skeleton> -->
        </el-card>
      </el-col>
  ...
  transition: {
    ...
    delay: 200
        ...
        <el-card shadow="never" class="min-h-[780px]">
          <template #header>
            <span class="cardheardtitle">Single-cell datasets</span>
          </template>
          <template #default>
  ...
  transition: {
    delay: 200
          ...
          <umap v-if="browsefiesttable" :DataSet="dataset" @getData="getData" />
        </el-col>
      </el-row>
  ...
  transition: {
    delay: 200
      ...
      <BrowseImg v-if="browsefiesttable" :DataSet="dataset" />
  ...
  transition: {
    delay: 200
          ...
          <detailcommunication v-if="browsefiesttable" :DataSet="dataset" />
        </el-col>
      </el-row>
    </div>
  </div>
