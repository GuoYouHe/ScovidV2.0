...
onMounted(async () => {
  ...
  setTimeout(() => {
    if (props.Data == '1') {
      (proxy.$refs["treeRef"] as any).setCurrentKey('Delorey TM et al.(Trachea)', true)

      // const nodes = ((
      //   proxy.$refs["treeRef"] as any
      // ).store._getAllNodes()[1].expanded = true);
    }
    (proxy.$refs["treeRef"] as any).setCurrentKey('Delorey TM et al.(Trachea)', true)
    // const nodes = ((
    //   proxy.$refs["treeRef"] as any
    ...
    emitter.on("sctreeexpand", ({ expandname, expandstatus }) => {
      setTimeout(() => {
        const nodes = ((proxy.$refs["treeRef"] as any).store.getNode({
          name: expandname
...
<template>
  <div class="h-full min-h-[480px] bg-bg_color overflow-auto">
    ...
    <el-divider />
    <el-tree class="treeclass" ref="treeRef" :data="treeData" node-key="name" size="small" :props="defaultProps"
      :default-expanded-keys="['']" :expand-on-click-node="false" :filter-node-method="filterNode" @node-click="nodeClick"
