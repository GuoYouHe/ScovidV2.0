...
import { getbrowsescabundance, getbrowsescumap } from "@/api/browse";
...
function useechart(id: any, data: any) {
  ...
  var option = {
    ...
    xAxis: {
      type: "category",
      // name: "x轴",
      data: data.sampleList, // 使用比较函数对字符串数组进行排序
      axisLabel: {
        //坐标轴刻度标签的相关设置。
        interval: 0,
        rotate: "45"
        ...
        <el-tabs type="border-card" v-model="tabPosition" @tab-click="pan">
          ...
          <el-tab-pane label="Cell type" name="celltype">
            ...
            <div class="con-leftCon-titleone-pic">
              <span>Figure 2A.Cell proportion plot that displays the proportion of each cell types per sample in the
                selected dataset.</span>
            </div>
          </el-tab-pane>
          <el-tab-pane label="Sub type" name="subtype">
            <div v-loading="loading" element-loading-text="Loading..." class="umap-img">
              <div ref="abundanceChartRef2" style="width: 100%; height: 100%" />
            </div>
            <!-- echart图底部标题 -->
            <div class="con-leftCon-titleone-pic">
              <span>Figure 2B. Cell proportion plot that displays the proportion of each sub types per sample in the
                selected dataset.</span>
            </div>
          </el-tab-pane>
        </el-tabs>
