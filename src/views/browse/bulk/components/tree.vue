...
onMounted(async () => {
  ...
  setTimeout(() => {
    ...
    emitter.on("bulktreeexpand", ({ expandname, expandstatus }) => {
      setTimeout(() => {
...
<template>
  <div class="h-full h-[700px] bg-bg_color overflow-auto">
    ...
    <el-tree class="treeclass" ref="treeRef" :data="treeData" node-key="name" size="small" :props="defaultProps"
      :default-expanded-keys="expandnum" :expand-on-click-node="false" :filter-node-method="filterNode"
      @node-click="nodeClick" v-loading="loading" element-loading-text="Loading...">
